...
const MineGrid: React.FC<MineGridProps> = ({ gridState }) => {
    ...
    return (
        <div className="grid grid-cols-5 gap-2 aspect-square p-2 bg-black/30 rounded-lg border border-white/10">
            {gridState.map((state, index) => (
                <div
                    ...
                    ${state === 'hidden' ? 'bg-black/30 hover:bg-accent-purple/20 hover:scale-105 cursor-pointer shadow-inner' : 'bg-white/5 scale-105'}
                    `}
                >
